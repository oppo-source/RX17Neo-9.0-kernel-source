...
static int synaptics_auto_test_rt59(struct seq_file *s, struct chip_data_s3706 *chip_info, struct syna_testdata *syna_testdata, uint8_t *data_buf)
{
        ...
        for (i = 0; i < node_num; i++) {
                ...
                if (max_store[i] > judge_threshold) {
...
static int synaptics_auto_test_rt63(struct seq_file *s, struct chip_data_s3706 *chip_info, struct syna_testdata *syna_testdata, uint8_t *data_buf)
{
        ...
        uint8_t raw_data_1[200] = {0};
        uint8_t raw_data_2[200] = {0};
        uint8_t analog_par_1[9] = {0x04, 0x01, 0x07, 0x01, 0x1F, 0x1F, 0x00, 0x00, 0x07};
        uint8_t analog_par_2[9] = {0x04, 0x00, 0x07, 0x01, 0x17, 0x17, 0x00, 0x00, 0x07};
        int32_t node_data_1 = 0, node_data_2 = 0;
        ...
        ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x01);        /* page 1*/
        ret = touch_i2c_write_block(chip_info->client, 0x36, sizeof(analog_par_1), analog_par_1);
        ret = touch_i2c_write_block(chip_info->client, 0x37, sizeof(analog_par_2), analog_par_2);
        ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x04);    /* force update*/
        checkCMD(chip_info, 30);
        while (value < 2) {
                ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x04);        /* page 4*/
                ret = touch_i2c_write_byte(chip_info->client, 0x2A, value);
                ret = touch_i2c_write_byte(chip_info->client, 0xff, 0x01);      //set page 1
                ret = touch_i2c_write_byte(chip_info->client, chip_info->reg_info.F54_ANALOG_COMMAND_BASE, 0x04);    /* force update*/
                checkCMD(chip_info, 30);
                ...
                checkCMD(chip_info, 30);
                if (!value) {
                        ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE+3, node_num*4, raw_data_1);         /*read data*/
                } else {
                        ret = touch_i2c_read_block(chip_info->client, chip_info->reg_info.F54_ANALOG_DATA_BASE+3, node_num*4, raw_data_2);         /*read data*/
                }
                value++;
        }

        for (i = 0; i < syna_testdata->RX_NUM; i++) {
            node_data_1 = (raw_data_1[i*4] | (raw_data_1[i*4+1]  << 8) | (raw_data_1[i*4+2]  << 16) | (raw_data_1[i*4+3]  << 24));
            node_data_2 = (raw_data_2[i*4] | (raw_data_2[i*4+1]  << 8) | (raw_data_2[i*4+2]  << 16) | (raw_data_2[i*4+3]  << 24));
            if (syna_testdata->fd >= 0) {
                    sprintf(data_buf, "[%d, %d]", node_data_1, node_data_2);
                    sys_write(syna_testdata->fd, data_buf, strlen(data_buf));
            }
            if (node_data_2 - node_data_1 < HOVER_THRESHOLD[i]) {
                TPD_INFO("Check RT63 for hover, data_1[%d] = %d, data_2[%d] = %d(%d)\n", i, node_data_1, i, node_data_2, HOVER_THRESHOLD[i]);
                if (!error_count)
                        seq_printf(s, "Check RT63 for hover, data_1[%d] = %d, data_2[%d] = %d(%d)\n", i, node_data_1, i, node_data_2, HOVER_THRESHOLD[i]);
                error_count++;
            }
        }
        if (syna_testdata->fd >= 0) {
