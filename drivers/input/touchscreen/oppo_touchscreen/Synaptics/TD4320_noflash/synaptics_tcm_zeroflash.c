...
void zeroflash_check_download_config(void)
{
    ...
    TPD_INFO("fw status:need app:%d need display:%d\n", g_zeroflash_hcd->fw_status.need_app_config,
...
static int zeroflash_download_app_config(void)
{
    ...
    TPD_INFO("Downloading application config\n");
...
static void zeroflash_download_config_work(struct work_struct *work)
{
    ...
    TPD_INFO("Start of config download\n");
    TPD_INFO("fw status:need app:%d need display:%d\n", g_zeroflash_hcd->fw_status.need_app_config,
        g_zeroflash_hcd->fw_status.need_disp_config);
...
exit:
    TPD_INFO("End of config download\n");
...
static int zeroflash_download_app_fw(void)
{
    ...
    TPD_INFO("Downloading application firmware\n");
...
reload_fw:
    ...
    TPD_INFO("Start of firmware download\n");
    ...
    TPD_INFO("End of firmware download\n");
...
exit:
    ...
    if (DOWNLOAD_RETRY_COUNT && retry_count > DOWNLOAD_RETRY_COUNT) {
        ...
        TPD_INFO(
                            "zeroflash_download_firmware_work disable_irq \n");
        ...
        enable_irq(tcm_hcd->s_client->irq);
            ...
            TPD_INFO(
                    "zeroflash_download_firmware_work enable_irq \n");
    }
