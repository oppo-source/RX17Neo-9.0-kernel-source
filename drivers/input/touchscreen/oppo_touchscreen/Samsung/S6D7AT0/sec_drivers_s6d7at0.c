...
static int sec_print_selftest_report(struct chip_data_s6d7at0 *chip_info, struct sec_testdata *sec_testdata, int test_type, u8 test_result)
{
  ...
  if (!print_buff)
	...
	break;
	case TOUCH_SELFTEST_ITEM_HIGH_Z:
		addr = SEC_FLASH_SELFTEST_REPORT_ADDR + (SELFTEST_FRAME_MAX_SIZE * 15);
	  TPD_INFO("%s: High-Z: %s\n", __func__, SELFTEST_RESULT(test_result, test_type));
		store_to_file(sec_testdata->fd, "%s: High-Z: %s\n", __func__, SELFTEST_RESULT(test_result, test_type));
	break;
...
//   sec_fix_tmode(chip_info, TOUCH_SYSTEM_MODE_TOUCH, TOUCH_MODE_STATE_TOUCH);
    touch_i2c_write_block(chip_info->client, SEC_CMD_MIS_CAL_CHECK, 0, NULL);    //set to verify calibration
    //sec_mdelay(100);
    //get calibration result, 0x0F:FAIL(bit[0]:data condition FAIL, bit[1]:RX gap FAIL, bit[2]:TX gap FAIL, bit[3]:TX/RX peak FAIL)

	ret = sec_wait_for_ready(chip_info, SEC_VENDOR_ACK_MIS_CAL_CHECK_DONE);
	if (ret < 0) {
      TPD_INFO("%s: Wait for mis cal done ack failed\n", __func__);
	}
